...
// DefaultTitleRules returns the v1 title/branch prefix rules.
...
func DefaultBodyRules() []BodyRule {
	...
	breakingRE := regexp.MustCompile(`(?i)\b(breaking|breaking.?change|api.?change|interface.?change| backwards.?incompatible)\b`)
	deprecationRE := regexp.MustCompile(`(?i)\b(deprecate|deprecated|remove.?in|will.?remove)\b`)
	perfRE := regexp.MustCompile(`(?i)\b(perf|performance|optimi[zs]|latency|throughput|faster|slower|memory.?leak)\b`)
	docsRE := regexp.MustCompile(`(?i)\b(docs?|documentation|readme|changelog|comment)\b`)
	testsRE := regexp.MustCompile(`(?i)\b(tests?|testing|test.?cov|unit.?test|integration.?test|e2e|spec)\b`)
	proxyRE := regexp.MustCompile(`(?i)\b(proxy|gate.?way|gost|quic|tunnel|reverse.?proxy|load.?balanc)\b`)
	infraRE := regexp.MustCompile(`(?i)\b(infra|infrastructure|nginx|systemd|deploy|container|docker|kubernetes|terraform|ansible)\b`)
	dbRE := regexp.MustCompile(`(?i)\b(schema|migration|postgres|mysql|mongo|redis|orm|database|table|column|index|foreign.?key)\b`)
	ciRE := regexp.MustCompile(`(?i)\b(ci|cd|pipeline|github.?action|gitlab.?ci|jenkins|workflow|build|test|lint)\b`)
	hardeningRE := regexp.MustCompile(`(?i)\b(harden|security.?control|tls|ssl|cert|mfa|2fa|rate.?limit|input.?valid|cors|csp)\b`)
	...
	return []BodyRule{
		...
		{Keyword: "[SECURITY]", Label: "security", Category: "security", Weight: 0.6, CaseInsensitive: true},
		{Keyword: "BREAKING", Label: "breaking", Category: "breaking", Weight: 0.6, CaseInsensitive: true},
	}
}

// DefaultPathRules returns the v1 file path rules.
func DefaultPathRules() []PathRule {
	return []PathRule{
		...
		{Glob: "**/*.js",              Label: "lang:js",     Lang: "JavaScript", Weight: 0.2},
		...
		{Glob: "Jenkinsfile*",        Label: "ci",           TouchesCI: "jenkins", Weight: 0.3},
		{Glob: "Makefile",            Label: "ci",           TouchesCI: "make", Weight: 0.2},
		// Docs-only
		{Glob: "*.md",                Label: "docs-only",     Weight: 0.2},
		{Glob: "docs/**",             Label: "docs-only",     Weight: 0.2},
		{Glob: "README*",             Label: "docs-only",     Weight: 0.2},
	}
}

// DefaultEffortRules returns the v1 effort/size rules.
